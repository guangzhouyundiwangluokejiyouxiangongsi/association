...
<meta http-equiv="keywords" content="<?php echo ($article["keywords"]); ?>" />
...
<body  > 
<style>
	.scroll_ulss li a p:hover{color: red !important}
</style>
<!-- 使用动画效果必须加上实例化 -->
 ...
 <script type="text/javascript">
  ...
  <!-- benner部分 -->
    ...
    <div class="Contactusb">
      <p>当前位置：<a target="_blank" href="<?php echo U('/Home/demeanour');?>">商会资讯</a> > <a href="javascript:;" onclick="jumps('<?php echo ($article_cat); ?>')"><?php echo ($article_cat); ?></a></p>
      <script>
        function jumps(cat)
        {
            if (cat == '最新动态') {
              window.open("<?php echo U('/Home/demeanourmoll');?>");
            }
            if (cat == '最新资讯'){
              window.open("<?php echo U('/Home/demeanour');?>");
            }
            if (cat == '公告'){
              window.open("<?php echo U('/Home/gonggao');?>");
            }
        }
      </script>
      <div class="Chamber">
    ...
    <div class="newsnext">
      <?php if($last): ?><p>上一篇:<a target="_blank" href="<?php echo U('/Home/news',array('new_id'=>$last['article_id']));?>"><?php echo (getSubstr($last["title"],0,14)); ?></a></p>
      <?php else: ?>
      <p>上一篇:&nbsp;&nbsp;没有了</p><?php endif; ?>

      <?php if($next): ?><p>下一篇:<a target="_blank" href="<?php echo U('/Home/news',array('new_id'=>$next['article_id']));?>"><?php echo (getSubstr($next["title"],0,14)); ?></a></p>
      <?php else: ?>
